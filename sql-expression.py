...
# linking chinook to python(/// means local database)
...
# create variable for the artist table (find coloumn names with "SELECT * FROM "artist" WHERE false;" in psql terminal)")
artist_table = Table(
    "artist", meta, 
...
# create variable for "Album" table (as artist id uses foreign key artist_id, we need to use ForeignKey)
album_table = Table(
    "album", meta,
